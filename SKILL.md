...
- “无二维码”竖版的标题和介绍文案必须复用 `template_5` 的标题、文案坐标和自动外部留白规则。当前底图为用户于 2026-09-17 提供的最新版 `template_new_no_qr.png`。其直播时间胶囊坐标为 `[882, 2304, 1328, 198]`，文案内容下边界为 `2293`；底图已预留标题、介绍文案和直播时间的位置，其他动态内容的填充坐标、字号及自动留白规则不调整。
...
- “横版”使用用户在 2026-09-14 最新提供的 `template_new_horizontal.png`。本次只微调固定设计细节；画布尺寸以及标题、介绍文案、直播时间的坐标、字号和自动留白规则均**不改**。其余主讲人、直播时间标签和获取链接等固定设计元素必须保留。
- 当前横版底图为用户于 2026-09-17 提供的最新版，已预留标题和介绍文案区域，不含示例动态文字。生成 `template_horizontal` 时直接在预留区域绘制；验收时不得出现旧版示例文字、白色残边、边框缺口或被截断的装饰圆形。
- 有二维码图的直播时间胶囊坐标为 `[882, 2378, 1328, 198]`。清除其中的示例日期时，白色覆盖仅限中间文字区 `x=1010–2100`，且上下各内缩 `24px`；不得覆盖胶囊的蓝色描边或两侧圆角。
- 标题中含中文逗号 `，` 时，四张海报都必须在第一个逗号后强制换行（例如 `美元加息落地，\n对我们投资有啥影响？`）。保持标题设计字号，不能为改行而缩小字号；生成前把换行后的同一标题传给全部 4 个 `create_poster()` 调用。
- 有二维码图的现行内容组基线为 `t5_content_shift=10` 和 `content_y_offset=-10`：相对原版，标题及整组介绍文案均上移 `20px`；二维码、关键词、直播时间胶囊和时间文字都不得移动。由于自动留白会抵消一半标题位移，**只改 `title_y` 或 `t5_content_shift` 不能满足此规则**，必须同时保留上述文案补偿。
