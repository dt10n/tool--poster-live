...
- “无二维码”竖版的标题和介绍文案必须复用 `template_5` 的标题、文案坐标和自动外部留白规则。当前底图为用户于 2026-09-14 提供的最新版 `template_new_no_qr.png`。底图已预留标题、介绍文案和直播时间的位置；只更新固定底图，不调整动态内容的填充坐标、字号及自动留白规则。
