...
- 课程群后同步群（不 @ 任何人）：学院直播预告帖正文、学院海报图片
...
226课程群三条消息全部发送并逐条核验 sender 后，必须紧接着把直播预告同步到另一个群。
...
- 课程群发布确认后：继续发完整 226课程群消息、同步发 `oc_e14b3cdc1aac37eeb1cb6dd23fadb70c` 群、创建团队群17:00定时任务、最终备份
...
- 从海报确认后开始，必须维护本期期数执行清单：
  ...
  4. 课程群后同步群 `oc_e14b3cdc1aac37eeb1cb6dd23fadb70c` 已发送学院直播预告帖正文和学院海报，并逐条核验 sender；
  5. 团队群 17:00 定时任务已创建并验证，或若已过 17:00，先询问用户是否立即补发；
  6. SKILL 当前最新期数和已确认记录已更新；
  7. 完整压缩包备份已生成。
